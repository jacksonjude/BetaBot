 ...
 */
module.exports.getShortcode = (emoji, addColons = true, stripVariations = false) => {
  emoji = emoji.trim();
  stripVariations && (emoji = emoji.normalize('NFKD').replace(/[\uFE0F\u20E3]/g, ''));

  //Check if a shortcode exists for the given character
  var shortcode = Object.keys(emojis).find(k => (stripVariations ? emojis[k].normalize('NFKD').replace(/[\uFE0F\u20E3]/g, '') : emojis[k]) === emoji);
 ...
 */
module.exports.demojify = (str, stripVariations) => {
  ...
  var result = str.replace(emojiRegex, (emoji) => {
    try {
      return module.exports.getShortcode(emoji, true, stripVariations);
    } catch (err) {
      if (err instanceof EmojiError) {
        return emoji;
      }
